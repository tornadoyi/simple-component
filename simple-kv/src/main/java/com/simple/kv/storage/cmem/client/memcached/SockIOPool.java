...
public class SockIOPool {
	...
	private long maxIdle              = 1000 * 60 * 2;		// max idle time for avail sockets
	private long maxBusyTime          = 1000 * 30;			// max busy time for busy sockets
	...
	// config for MAX_RETRY_DELAY,if this value is not zero, use this value
	private long configuredMaxRetryDelay = 0L;
	...
	public long getConfiguredMaxRetryDelay() {
		return configuredMaxRetryDelay;
	}

	public void setConfiguredMaxRetryDelay(long configuredMaxRetryDelay) {
		this.configuredMaxRetryDelay = configuredMaxRetryDelay;
	}
	...
	protected SockIO createSocket( String host ) {
		...
		try {
			...
			log.error( "++++ failed to get SockIO obj for: " + host + ", detail:" + ex.getMessage(), ex);
			socket = null;
		}
		...
		try {
			if ( socket == null ) {
				...
				long realMaxRetryDelay = this.configuredMaxRetryDelay > 0 ? this.configuredMaxRetryDelay : MAX_RETRY_DELAY;
				if (expire > realMaxRetryDelay)
					expire = realMaxRetryDelay;
			...
			if ( availPool != null && !availPool.isEmpty() ) {
				...
				if ( aSockets != null && !aSockets.isEmpty() ) {
					...
					// We can't use iterator.remove on an IdentityHashMap due to bug 6612102 so let's get the removeSet and remove them later.
				 	Set<SockIO> removeSet = Collections.newSetFromMap(new IdentityHashMap<SockIO,Boolean>());
					try{
				...
				try {
					for ( Iterator<SockIO> j = sockets.keySet().iterator(); j.hasNext(); ) {
						...
						if ( (expire + maxIdle) < System.currentTimeMillis() ) {
							if ( log.isDebugEnabled() )
								log.debug( "+++ removing stale entry from pool as it is past its idle timeout" );
