 ...
 */
package com.simple.kv.storage.cmem.client.memcached;
...
public class MemcachedClient {
	...
	public boolean delete(String key, Integer hashCode, Date expiry) {
		...
		try {
			...
			if (DELETED.equals(line)) {
				...
				log.error("++++ cmem server response: " + line);
				throw new RuntimeException("cmem client error, key: " + key + ", msg: " + line);
			} else if (line.startsWith(SERVER_ERROR)) {
				log.error("++++ cmem server response: " + line);
				throw new RuntimeException("delete from cmem error, key: " + key + ", msg: " + line);
			} else {
				log.error("++++ cmem crosstalk when delete " + ", key:" + key + ", response:" + line);
				throw new RuntimeException("delete from cmem error, key: " + key + ", msg: " + line);
			}
		} catch (Exception e) {
			...
			try {
				...
				log.error("++++ failed to close socket : " + sock.toString(), e);
			} finally {
				sock = null;
			}
			...
			if (e instanceof RuntimeException) {
				...
				log.error("exception thrown while delete, " + e.getMessage(), e);
	...
	private boolean set(String cmdname, String key, Object value, Date expiry,
		...
		if (NativeHandler.isHandled(value)) {
			...
			if (asString) {
				...
				try {
					if (log.isDebugEnabled())
						log.debug("++++ storing data as a string for key: "
								+ key + " for class: "
			...
			try {
				if (log.isDebugEnabled())
					log.debug("++++ serializing for key: " + key
		...
		if (compressEnable && val.length > compressThreshold) {
			...
			try {
				...
				if (log.isDebugEnabled())
					log.debug("++++ compression succeeded, size after: "
							+ val.length);
			} catch (IOException e) {
		...
		try {
			...
			if (STORED.equals(line)) {
				...
				log.error("++++ data not stored in cache for key: " + key);
				throw new RuntimeException("cmem data not stored, key: " + key + ", msg: " + line);
			} else if (line.startsWith(CLIENT_ERROR)) {
				log.error("++++ cmem server response: " + line);
				throw new RuntimeException("cmem client error, key: " + key + ", msg: " + line);
			} else if (line.startsWith(SERVER_ERROR)) {
				log.error("++++ cmem server response: " + line);
				throw new RuntimeException(cmdname + " to cmem error, key: " + key + ", msg: " + line);
			} else {
				log.error("++++ cmem crosstalk when " + cmdname + ", key:" + key + ", response:" + line);
				throw new RuntimeException(cmdname + " to cmem error, key: " + key + ", msg: " + line);
			}
		} catch (Exception e) {
			...
			if (e instanceof RuntimeException) {
				...
				log.error("exception thrown while " + cmdname + ", " + e.getMessage(), e);
				throw new RuntimeException(e);
			}
		} finally {
			if (sock != null) {
	...
	private long incrdecr(String cmdname, String key, long inc, Integer hashCode) {
		...
		try {
			...
			if (line.matches("\\d+")) {
				...
				log.error("++++ cmem server response: " + line);
				throw new RuntimeException("cmem client error, key: " + key + ", inc:" + inc + ", msg: " + line);
			} else if (line.startsWith(SERVER_ERROR)) {
				log.error("++++ cmem server response: " + line);
				throw new RuntimeException(cmdname + " to cmem error, key: " + key + ", inc:" + inc + ", msg: " + line);
			} else {
				log.error("++++ cmem crosstalk when " + cmdname + ", key:" + key + ", response:" + line);
				throw new RuntimeException(cmdname + " to cmem error, key: " + key + ", msg: " + line);
			}
	 ...
	 * @return
	 */
	public Object getExt(String key, Integer hashCode) {
		return getExt(key, hashCode, false);
	}
	 ...
	 * @return
	 */
	public Object getExt(String key, Integer hashCode, boolean asString) {
		return this.doGet("get_ext", key, hashCode, asString, true);
	}
	...
	public Object get(String key, Integer hashCode, boolean asString) {
		return doGet("get", key, hashCode, asString, false);
	}

	/**
	 * deal the get and getExt
	 * 
	 * @param command
	 * @param key
	 * @param hashCode
	 ...
	 * @return
	 */
	public Object doGet(String command, String key, Integer hashCode, boolean asString, boolean checkResult) {
		...
		try {
			...
			while (true) {
				...
				if (line.startsWith(VALUE)) {
					...
					String getKey = info[1];
					
					if(!key.equals(getKey)) {
						log.error("++++ cmem crosstalk when " + command + ", key: " + key + ", real: " + getKey);
						throw new RuntimeException(command + " from cmem crosstalk, key: " + key + ", real: " + getKey);
					}
					...
					if (checkResult) {
						...
						flag = Integer.parseInt(info[3]);
						length = Integer.parseInt(info[4]);
					} else {
						flag = Integer.parseInt(info[2]);
					...
					if (checkResult && result != 0) {
						String data = new String(buf, this.defaultEncoding);
						StringBuilder errorMessage = new StringBuilder();
						errorMessage.append("{key:\"")
								.append(key).append("\", code:\"").append(result).append("\", msg:\"").append(data).append("\"}");
						log.error(command + " from cmem error: " + errorMessage.toString());
					...
					// we can only take out serialized objects, no serialized == primitive type
					if ((flag & F_SERIALIZED) != F_SERIALIZED) {
						...
						// deserialize if the data is serialized, serialized == object
					...
					log.error("++++ cmem server response: " + line);
					throw new RuntimeException("cmem client error, msg: " + line);
				} else if (line.startsWith(SERVER_ERROR)) {
					log.error("++++ server response: " + line);
					throw new RuntimeException(command + " from cmem error, key: " + key + ", msg: " + line);
				}
			}
			...
			if (e instanceof RuntimeException) {
				...
				log.error("exception thrown while " + command + ", " + e.getMessage(), e);
				throw new RuntimeException(e);
			}
	...
	/**
	 * Retrieve multiple keys from the memcache and detect the error
	 * 
	 * @param keys
	 * @param hashCodes
	 * @param asString
	...
	public Map<String, Object> getMultiExt(String[] keys) {
		return this.getMultiExt(keys, null, false);
	}
	 ...
	 * @return
	 */
	public Map<String, Object> getMultiExt(String[] keys, Integer[] hashCodes, boolean asString) {
		return this.dealMulti("get_ext", keys, hashCodes, asString, true);
	}
	...
	public Map<String, Object> getMulti(String[] keys, Integer[] hashCodes,
			boolean asString) {
		return this.dealMulti("get", keys, hashCodes, asString, false);
	}

	public Map<String, Object> dealMulti(String command, String[] keys, Integer[] hashCodes,
			boolean asString, boolean checkResult) {
		...
		for (int i = 0; i < keys.length; ++i) {
			...
			if (!cmdMap.containsKey(sock.getHost()))
				cmdMap.put(sock.getHost(), new StringBuilder(command));// /the command
		...
		(new NIOLoader(this)).doMulti(asString, cmdMap, keys, ret, checkResult);
	...
	private void loadMulti(LineInputStream input, Map<String, Object> hm,
			boolean asString, boolean checkResult) throws IOException {
		...
		while (true) {
			...
			if (line.startsWith(VALUE)) {
				...
				if (checkResult && result != 0) {
					String data = new String(buf, this.defaultEncoding);
					StringBuilder errorMessage = new StringBuilder();
					errorMessage.append("{key:\"").append(key).append("\", code:\"").append(result).append("\", msg:\"").append(data).append("\"}");
					log.error((checkResult ? "bget_ext" : "get_ext") + " from cmem error: " + errorMessage.toString());
					throw new RuntimeException(errorMessage.toString());
				}
				// ready object
				Object o;
				...
				log.error("++++ cmem server response: " + line);
				throw new RuntimeException("bget from cmem error, msg: " + line);
			}
		}
	}
		...
		for (int i = 0; i < servers.length; i++) {
			...
			try {
				...
				try {
					...
					sock.trueClose();
					return this.flushAll(servers);
				}
				success = (OK.equals(line)) ? success && true : false;
			} catch (IOException e) {

				// exception thrown
				log.error("exception thrown while writing bytes to server on flushAll", e);
	...
	/**
	 * return the value and detect the error
	 * 
	 * @param key
	 * @return
	 */
	public CASValue getsExt(String key) {
		return this.doGets("gets_ext", key, true);
	}
	...
	public CASValue gets(String key) {
		return this.doGets("gets", key, false);
	}

	/**
	 * deal the gets and gets_ext
	 * 
	 * @param command
	 * @param key
	 * @param checkResult
	 * @return
	 */
	public CASValue doGets(String command, String key, boolean checkResult) {
		if (key == null) {
			log.error("key is null for gets()");
			return null;
		}
			...
			while (true) {
				...
				if (line.startsWith(VALUE)) {
					...
					if (checkResult) {
						...
						flag = Integer.parseInt(info[3]);
						length = Integer.parseInt(info[4]);
						casUnique = Long.parseLong(info[5]);
					} else {
						flag = Integer.parseInt(info[2]);
						length = Integer.parseInt(info[3]);
						casUnique = Long.parseLong(info[4]);
					}
					if (log.isDebugEnabled()) {
						log.debug("++++ key: " + key);
						if (checkResult) {
					...
					response = new CASValue(o, casUnique);
				} else if (END.equals(line)) {
					if (log.isDebugEnabled())
						log.debug("++++ finished reading from cache server");
					break;
	...
	public CASResponse cas(String key, long casId, Object value) {
		return cas(key, casId, value, null);
	}

	public CASResponse cas(String key, long casId, Object value, Date expiry) {
		if (key == null) {
			log.error("key is null for gets()");
			return null;
		}
		...
		// now write the data to the cache server
		try {
			String cmd = String.format("cas %s %s %d %d %d\r\n", key, flags, (expiry.getTime() / 1000),
					val.length, casId);
			sock.write(cmd.getBytes());
			sock.write(val);
			sock.write("\r\n".getBytes());
			...
			if (STORED.equals(line)) {
				...
				if (log.isDebugEnabled())
					log.debug("++++ data not found in cache for key: " + key);
				response = CASResponse.NOT_FOUND;
			} else if ("EXISTS".equals(line)) {
				if (log.isDebugEnabled())
					log.debug("++++ data exists in cache for key: " + key);
				...
				log.error("++++ cmem server response: " + line);
				throw new RuntimeException("cas to cmem error, key: " + key + ", msg: " + line);
			} else {
				log.error("++++ cmem crosstalk when cas, key:" + key + ", response:" + line);
				throw new RuntimeException("cas to cmem error, key: " + key + ", msg: " + line);
			}
			return response;
		} catch (Exception e) {
			...
			if (e instanceof RuntimeException) {
				...
				log.error("exception thrown while cas, " + e.getMessage(), e);
				throw new RuntimeException(e);
			}
		} finally {
			if (sock != null) {
	...
	protected final class NIOLoader {
		...
		private final class Connection {
			...
			public Connection(SockIOPool.SockIO sock, StringBuilder request)
					throws IOException {
				if (log.isDebugEnabled())
			...
			public void close() {
				try {
					if (isDone) {
						...
						if (log.isDebugEnabled())
							log.debug("++++ gracefully closing connection to "
									+ sock.getHost());
				...
				try {
					if (log.isDebugEnabled())
						log.debug("forcefully closing connection to "
								+ sock.getHost());
			...
			public String toString() {
				return "Connection to " + sock.getHost() + " with "
						+ incoming.size() + " bufs; done is " + isDone;
			}
		}

		public void doMulti(boolean asString,
				Map<String, StringBuilder> sockKeys, String[] keys,
				Map<String, Object> ret, boolean checkResult) {// sockkeys:serverHost->get cleanKey
		...
		private void handleKey(SelectionKey key) throws IOException {
			if (log.isDebugEnabled())
				log.debug("handling selector op " + key.readyOps()
						+ " for key " + key);
			...
			if (count > 0) {
				if (log.isDebugEnabled())
					log.debug("read  " + count + " from "
							+ conn.channel.socket().getInetAddress());
